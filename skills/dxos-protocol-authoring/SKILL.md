...
1. 引用标记 `@图[N]`（画布编辑器内部 token）→ `@图片k` / `@视频k`（接口要求的引用名）。
   ⚠️ **这是「按素材类型各自独立编号」的重写，不是简单换字**：`N` 是**附件挂载序号**（第几个挂上去的），
   `k` 是**该类型内的序号**。实测（2026-09-15）同一张卡挂 [场景图, 参考视频, 人物图]：
   `@图[1]→@图片1`、`@图[2]→@视频1`、`@图[3]→@图片2` —— 所以 `@图[3]` 可能变成 `@图片2` 或 `@视频1`，
   取决于第 3 个挂的是图片还是视频。**核对顺序时看 body 里数组的下标，不要看 `@图[N]` 的 N。**
   恰好命中「不传 `name` 时素材自动命名为 `图片N`/`视频N`」的约定，所以是**对的**，
   不要在协议里再画蛇添足去解析 `@图[N]`。
2. 末尾多余空行被裁掉。中间的 `\n` 与前后空格**会原样保留**（例：`这个场景\n 下参考…` 原样发出）。

⚠️ 画布库里的编辑器文本是**实时**的：用户在提交之后又改过卡（删素材、重写文案）就会失真。
判据永远以 `plan_json` 的 `submit.request.body` 为准，画布库只用来**交叉验证**；
两者对不上时先怀疑"卡被事后改过"，别急着判协议丢内容。

除这两处外应逐字节相同；若出现截断/转义损坏/字段丢失，才是真问题。

### 判据：报错是"参考素材敏感/审核未通过"→ 上游风控，协议不用改

`error_message` 出现 **`参考素材可能包含敏感内容，请更换后重试`** 或 **`内容审核未通过，请修改后重试`** 时：

| 看什么 | 结论 |
| --- | --- |
| `remote_task_id` 非空 + `poll_attempt` 十几个 | **提交是被接受的**（请求体与素材都送达了），死在**上游排队后的内容审核** |
| 事件时间线 progress 先涨（如 30）再 `failed` | 上游确实在跑，风控在生成阶段才拒 |
| 同一站点其他任务能出片 | 排除鉴权/渠道/协议，纯粹是**这批参考素材**被风控挑了 |

处置方向只有一个：**换参考素材/换模型重试**，协议一个字都不用改。
（实测 2026-09-15 aicost `seedance2.0-480p` multimodal：22:43:10 提交成功拿回 task id，
12 次轮询后 22:44:21 报此错；同站 seedance 系列 09-13 也有两次 `内容审核未通过`。）
